...
namespace ScheduleOne.NPCs.Behaviour
{
	public class CheckpointBehaviour : Behaviour
	{
		...
		{
			if (!InstanceFinder.IsServer || player.CrimeData.TimeSinceLastBodySearch < 60f || player.CrimeData.CurrentPursuitLevel != 0 || NetworkSingleton<CurfewManager>.Instance.IsCurrentlyActive || Checkpoint.AssignedNPCs.Count == 0)
			{
				return;
			}
