	...
	public class DryingRack : GridItem, IUsable, IItemSlotOwner, ITransitEntity, IConfigurable
	{
		...
		public void RefreshHangingVisuals()
		{
			...
			StoredItem[] array = (from x in HangingVisuals.ItemContainer.GetComponentsInChildren<StoredItem>()
				where !x.Destroyed
				select x).ToArray();
			for (int j = 0; j < array.Length && j < HangAlignments.Length; j++)
			{
				Transform transform = array[j].GetComponentsInChildren<Transform>().FirstOrDefault((Transform x) => x.name == "HangingAlignment");
				if (transform == null)
				{
					Console.LogError("Missing alignment transform on stored item: " + array[j].name);
					continue;
				}
				Transform obj = HangAlignments[j];
				Quaternion quaternion = obj.rotation * Quaternion.Inverse(transform.rotation);
				array[j].transform.rotation = quaternion * array[j].transform.rotation;
				Vector3 vector = obj.position - transform.position;
				array[j].transform.position += vector;
			}
		}
